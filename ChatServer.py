...
from protocol import sendObject, receiveObject#imports sendObject and receiveObject functions from protocol.py for sending and receiving messages
...
class ChatServer:
    ...
    def takingCommands(self, sock, user, obj):#processes the command received from the client
        ...
        if command=="nick":#if the command is to pick a nickname
            self.Nicknames(sock, user, args)#calls the handle_nick function to process the nickname command
        elif command=="list":#if the command is to list channels and the number of users in each channel
            self.List(sock, user)#calls the handle_list function to process the list command
        elif command=="join":#if the command is to join a channel
            self.Join(sock, user, args)#calls the handle_join function to process the join command
        elif command=="leave":#if the command is to leave a channel
            self.Leave(sock, user, args)#calls the handle_leave function to process the leave command
        elif command=="quit":#if the command is to disconnect from the server and leave the chat
        ...
        elif command=="help":#if the command is to show the help message
    ...
    def List(self, sock, user):#function to handle the /list command from the client
        with self.lock:#uses a threading lock to prevent data corruption when accessing shared resources
            NamesAndNumbers = {#creates a dictionary comprehension to store the channel names and the number of users in each channel
    ...
    def Leave(self, sock, user, args):#function to handle the /leave command from the client
        ...
        leavingChannels = [args[0]] if args else list(user["channels"])#determines if a user want to leave all or just one specific channel
